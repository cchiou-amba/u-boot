...
const struct pinmux_config cv3_init_pinmux[] = {
	/* UART APB */
...
{
	pinmux_config_set_item(cv3_init_pinmux, sizeof(cv3_init_pinmux));
}
...
void plat_f_soc_init(void)
{
	...
	writel(0, 0xfff3000090U);
	writel(0, 0xfff3000094U);
	writel(0, 0xfff3000098U);
	writel(0, 0xfff300009CU);
	writel(0, 0xfff30000A0U);
}
...
#define CORTEX_RVBARADDR0_REG           0xfff3000048
#define CORTEX_RESET_REG                0xfff3000028

static inline void BLD_Boot_ClusterCore(uint32_t ClusterID, uint32_t CoreID, uint32_t entry)
{
    writel(CORTEX_RVBAR_ADDR(gd->relocaddr), CORTEX_RVBARADDR0_REG + (CoreID << 2) + (ClusterID << 4));
    clrbits_32(CORTEX_RESET_REG, (0x1 << (CoreID + 3 + ClusterID * 7)));
}

void cpu_secondary_init_r(void)
{
    BLD_Boot_ClusterCore(0U, 1U, CONFIG_SYS_TEXT_BASE);
    BLD_Boot_ClusterCore(0U, 2U, CONFIG_SYS_TEXT_BASE);
    BLD_Boot_ClusterCore(0U, 3U, CONFIG_SYS_TEXT_BASE);
}

void plat_device_init(void)
{
	uint32_t test;
	/* clean UART fifo garbage left by dram training */
	test= readl(UARTD_BASE);
