...
static int regmap_field_read(struct regmap_field *field, unsigned int *val)
{
	int ret;
			      ...
			      struct reg_field reg_field)
{
	...
	rm_field->reg = reg_field.reg << 2;
	rm_field->shift = reg_field.lsb;
	rm_field->mask = GENMASK(reg_field.msb, reg_field.lsb);
}

static struct regmap_field *devm_regmap_field_alloc(struct udevice *dev,
					     struct regmap *regmap,
					     struct reg_field reg_field)
{
...
extern struct regmap *regmap_alloc(int count);
...
static struct regmap *amba_cdns3_phy_init_regmap(struct udevice *dev, void __iomem *base, size_t size)
{
	...
	map = regmap_alloc(1);
	if (!map)
		pr_err("amba_cdns3_phy_init_regmap alloc err, ret %ld\n", PTR_ERR(map));

	range = map->ranges;
	range->start = (u64)base;
	range->size = size;
...
extern int init_range(ofnode node, struct regmap_range *range, int addr_len,
				       ...
				       u8 reg_offset_shift)
{
	...
	return amba_cdns3_phy_init_regmap(dev, base + block_offset,
		sp->size - block_offset);
}
	...
	for (node = 0; node < cdns_phy->nsubnodes; node++) {
		...
		if (link_cmn_vals) {
			...
			for (i = 1; i < num_regs; i++)
				regmap_write(regmap, reg_pairs[i].off << 2,
					     reg_pairs[i].val);
		}
		...
		if (xcvr_diag_vals) {
			...
			for (i = 0; i < num_lanes; i++) {
				...
				for (j = 0; j < num_regs; j++)
					regmap_write(regmap, reg_pairs[j].off << 2,
						     reg_pairs[j].val);
			}
		}
...
static int cdns_torrent_phy_probe(struct udevice *dev)
{
	...
	if(NULL == data)
		printf("dev_get_driver_data err %d\n", __LINE__);

	cdns_phy->init_data = data;

	reset_get_by_index(dev, 0, cdns_phy->phy_rst);
...
static int cdns_torrent_phy_init(struct phy *phy)
{
	 ...
	 */
	writel((u64)cdns_phy->sd_base + 0x34000 + 0x4c, 0x1111);
#endif
	/**
